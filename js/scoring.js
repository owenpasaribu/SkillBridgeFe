...
function generateRoadmap(state) {
  ...
  const phases = gaps.map(g => {
    ...
    return {
      ...
      learningObjective: content.objective || `Meningkatkan kemampuan ${skillName(g.skillId)}.`,
      why: content.why || `Skill ini dibutuhkan untuk mencapai ${career.name}.`,
      after: content.after || 'Kamu akan lebih siap menghadapi tugas terkait skill ini.',
      resources,
      tasks: content.tasks || [],
  ...
  phases.push({
    ...
    priority: 'High',
    status: 'not_started',
    learningObjective: `Membangun satu proyek end-to-end yang merepresentasikan kesiapan kamu sebagai ${career.name}.`,
    why: 'Proyek nyata membuktikan skill yang sudah kamu pelajari, bukan cuma teori.',
    after: 'Kamu punya satu portofolio kuat yang bisa ditunjukkan ke recruiter.',
    resources: [],
    tasks: ['Tentukan studi kasus proyek', 'Kerjakan end-to-end dari data/desain sampai hasil akhir', 'Dokumentasikan di GitHub / portofolio'],
    miniProject: `Proyek akhir bertema ${career.name}.`,
    durationDays: 14,
  });
